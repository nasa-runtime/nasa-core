...
import static io.github.nasaruntime.core.concurrent.MPMCChunk.CHUNK_MASK;
import static io.github.nasaruntime.core.concurrent.MPMCChunk.CHUNK_SHIFT;
import static io.github.nasaruntime.core.concurrent.MPMCChunk.FREE;
...
public class MPMCLinkedQueue<E> extends MPMCQPad2<E> implements Queue<E>, Serializable {
    ...
    /**
     * 业务作用：报告块复用是否被关闭。关闭后每段队列都重新分配块，分配率上升但排除了块复用相关的问题，便于排障。
     *
     * 参数说明: 无。
     * 返回: 已关闭返回 true。
     */
    ...
    /**
     * 业务作用：按 Queue#add 契约入队，失败时抛异常而非返回 false，使调用方无法忽略写入失败。
     *
     * @param e 待入队元素，不允许为 null
    ...
    /**
     * 业务作用：给槽位打上毒标记，使该位置永久不可用。用于消费者发现槽位状态不可恢复时阻断后续读取，把静默的数据错乱转成可观测的失败。
     *
     * @param pIndex 生产者位置
     * 返回: 无返回值。
     */
    ...
    /**
     * 业务作用：按 Queue#remove 契约出队，空队列抛异常而非返回 null。
     *
     * 参数说明: 无。
    ...
    /**
     * 业务作用：读取队首元素但不摘除；返回后该元素可能已被其它消费者取走。
     *
     * 参数说明: 无。
     * 返回: 队首元素；队列为空返回 null。
     */
    ...
    /**
     * 业务作用：按 Queue#element 契约窥探队首，空队列抛异常而非返回 null。
     *
     * 参数说明: 无。
     * 返回: 队首元素；队列为空抛出 NoSuchElementException。
     */
    ...
    /**
     * 业务作用：提供可分割遍历器，声明并发特征使 Stream 使用方知晓遍历期间允许并发修改。
     *
     * 参数说明: 无。
     * 返回: 基于弱一致迭代器的 Spliterator。
     */
    @Override
    ...
    /**
     * 业务作用：批量入队。走 add 而非 offer：有界队列写满时必须让调用方感知失败，不能静默只加入一部分。
     *
     * @param c 待入队集合
     * 返回: 至少加入一个元素返回 true；中途写满会抛出 IllegalStateException 且不回滚。
     */
    @Override
