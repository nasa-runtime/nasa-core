...
public class IntegerArrayCopier extends ACopier {
    ...
    /**
     * 业务作用：预先算出装箱整数数组拼接成字符串后占用的字符数，供拼接方一次性分配足够大的字符数组，
    ...
    public int length(Object sources) {
        ...
        Integer[] ss = (Integer[]) sources;
        int length = 0;
        for (Integer source : ss) {
    ...
    /**
     * 业务作用：把装箱整数数组的字符表示写入目标字符数组的指定位置，是零中间字符串拼接的实际执行步骤。
