...
public class LongArrayCopier extends ACopier {

    private static ACopier copier;

    /**
     * 业务作用：给出本复制器在注册表中的类型键，Long[] 类型的源数据据此路由到本实现。
    ...
    /**
     * 业务作用：预先算出装箱长整数数组拼接成字符串后占用的字符数，供拼接方一次性分配足够大的字符数组，
    ...
    /**
     * 业务作用：把装箱长整数数组的字符表示写入目标字符数组的指定位置，是零中间字符串拼接的实际执行步骤。
