...
public class ZipOprException extends BaseException {
    ...
    /**
     * 业务作用：构造不带描述的压缩与解压异常，供调用方随后自行补充 code 与 message。
     *
     * 参数说明: 无。
    ...
    /**
     * 业务作用：以明确描述报告压缩与解压失败，用于归档格式非法、条目缺失或 IO 失败。
     *
     * @param msg 异常描述
    ...
    /**
     * 业务作用：以带占位符的描述报告压缩与解压失败，用于归档格式非法、条目缺失或 IO 失败。
    ...
    /**
     * 业务作用：在描述之外同时指定业务状态码，供上层按 code 分支处理压缩与解压失败。
     *
