...
public class FunctionException extends BaseException {
    ...
    /**
     * 业务作用：构造不带描述的函数式调用异常，供调用方随后自行补充 code 与 message。
     *
     * 参数说明: 无。
    ...
    /**
     * 业务作用：包装底层异常为函数式调用异常，保留原始 message 与 cause，使调用方不必感知底层实现类型。
     *
     * @param e 被包装的原始异常
    ...
    /**
     * 业务作用：以带占位符的描述报告函数式调用失败，用于被包装的函数在执行期抛错，需要转为非受检异常向上传播。
    ...
    /**
     * 业务作用：在描述之外同时指定业务状态码，供上层按 code 分支处理函数式调用失败。
     *
