...
- 修复排队任务取消、拒绝或冻结时终态先于业务所有权与逻辑计数发布，导致 consumer 可能提前释放任务对象的问题；终态发布期间通过不可执行中间态关闭并发执行和回收窗口。
- 修复队列发布失败与严格存量迁移异常路径中的逻辑计数重复递减、故障证据缺失和任务终态丢失。
- 修复严格任务在本地追赶阶段并发取消时可能错误冻结整个任务类型的问题。
- 修复严格热点长期被非严格候选压制的问题；每轮集中观察为严格候选保留有界、轮转的目标尝试机会。
